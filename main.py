...
class RespondedComment(db.Model):
    __tablename__ = 'responded_comments'
    id = db.Column(db.Integer, primary_key=True)
    comment_id = db.Column(db.String(255), unique=True, nullable=False)

def has_already_responded(comment_id):
    # Verifica se o comentário já foi respondido
    return db.session.query(RespondedComment).filter_by(comment_id=comment_id).first() is not None

def mark_as_responded(comment_id):
    # Marca o comentário como respondido
    responded_comment = RespondedComment(comment_id=comment_id)
    db.session.add(responded_comment)
    db.session.commit()
...
def alterar_status_usuario(id):
    ...
    if usuario.id == current_user.id:
        flash("Você não alterar estado da sua propia conta!", "danger")
        return redirect(url_for('listar_usuarios'))
    try:
...
def login():
    if request.method == 'POST':
        ...
        # Busca o usuário e verifica simultaneamente se está ativo
        usuario = User.query.filter_by(nome=nome, ativo=True).first()
        ...
        # Mensagem genérica para evitar expor informações sensíveis
        flash('Credenciais inválidas ou conta inativa.', 'danger')
...
def dashboard():
    try:
        ...
        # Variáveis para organizar os dados
        engagement_data = defaultdict(
            lambda: {"facebook_likes": 0, "facebook_comments": 0, "instagram_likes": 0, "instagram_comments": 0})

        # Calcula a data mínima (500 dias atrás)
        ...
        try:
            ...
            for post in fb_engagement_response.get("data", []):
                post_date = datetime.strptime(post["created_time"][:10], "%Y-%m-%d").date()
                if post_date >= min_date:  # Verifica se a data está no intervalo
                    date_str = post["created_time"][:10]
                    engagement_data[date_str]["facebook_likes"] += post.get("likes", {}).get("summary", {}).get(
                        "total_count", 0)
                    engagement_data[date_str]["facebook_comments"] += post.get("comments", {}).get("summary", {}).get(
                        "total_count", 0)
            ...
            for media in ig_engagement_response.get("data", []):
                media_date = datetime.strptime(media["timestamp"][:10], "%Y-%m-%d").date()
                if media_date >= min_date:  # Verifica se a data está no intervalo
                    date_str = media["timestamp"][:10]
                    engagement_data[date_str]["instagram_likes"] += media.get("like_count", 0)
                    engagement_data[date_str]["instagram_comments"] += media.get("comments_count", 0)

            # Ordena os dados por data
            sorted_dates = sorted(engagement_data.keys())
            labels = sorted_dates
            facebook_likes_data = [engagement_data[date]["facebook_likes"] for date in sorted_dates]
            facebook_comments_data = [engagement_data[date]["facebook_comments"] for date in sorted_dates]
            instagram_likes_data = [engagement_data[date]["instagram_likes"] for date in sorted_dates]
            instagram_comments_data = [engagement_data[date]["instagram_comments"] for date in sorted_dates]
...
# Emissão de novas mensagens em tempo real
def fetch_and_emit_new_messages():
    last_message_ids = {}  # Guarda IDs da última mensagem por conversa

    while True:
        messages = fetch_messages()
        for msg in messages:
            conversation_id = msg.get("conversation_id")
            last_msg_id = last_message_ids.get(conversation_id)

            # Verifica se é uma nova mensagem
            if last_msg_id != msg.get("id"):
                last_message_ids[conversation_id] = msg.get("id")  # Atualiza último ID
                socketio.emit('new_message', {"message": msg, "conversation_id": conversation_id})

        time.sleep(1)  # Verificação frequente para atualizar rapidamente

# Iniciar a thread para buscar novas mensagens
threading.Thread(target=fetch_and_emit_new_messages, daemon=True).start()
...
def delete_posts(post_id):
    ...
    if source.lower() == "facebook":
        ...
        url = f"https://graph.facebook.com/v16.0/17841465681073010?fields=media/{post_id}"
    else:
    ...
    if response.status_code == 200:
        ...
        print(response.json())  # Log do erro
        return {"error": response.json().get("error", {}).get("message", "Erro desconhecido")}, response.status_code
...
def like_comment(comment_id):
    try:
        # Simula a ação de "curtir" no backend (dependendo da API usada)
...
def get_comments(post_id):
    try:
        ...
        for comment in comments.get('data', []):
            replies = graph.get_connections(id=comment['id'], connection_name='comments')
            reply_data = []
            for reply in replies.get('data', []):
                reply_data.append({
                    'id': reply.get('id', 'N/A'),
                    'message': reply.get('message', 'Sem mensagem'),
                    'created_time': reply.get('created_time', 'Desconhecido'),
                    'from': reply.get('from', {}).get('name', 'Anônimo'),
                    'avatar': f"https://graph.facebook.com/{reply['from']['id']}/picture?type=square"
                })

            # Inclui os likes no comentário principal
            comment_likes = graph.get_object(id=comment['id'], fields='likes.summary(true)')
            likes_count = comment_likes.get('likes', {}).get('summary', {}).get('total_count', 0)
            ...
            comment_data.append({
                ...
                'likes': likes_count,  # Adiciona o número de likes
                'replies': reply_data
            })
...
def edit_post(post_id):
    try:
        ...
        remove_image = request.form.get('remove_image') in ['true', '1', 'True']
        ...
        if platform == "Facebook":
            if image or remove_image:
                # Excluir o post atual
                try:
                    graph.delete_object(post_id)
                except Exception as delete_error:
                    return {'error': f"Erro ao excluir post existente: {delete_error}"}, 500

                # Criar novo post com imagem ou sem imagem
                if image:
                    try:
                        graph.put_photo(image, message=message)
                    except Exception as photo_error:
                        return {'error': f"Erro ao criar novo post com imagem: {photo_error}"}, 500
                else:
                    try:
                        graph.put_object(parent_object="me", connection_name="feed", message=message)
                    except Exception as text_post_error:
                        return {'error': f"Erro ao criar novo post: {text_post_error}"}, 500
            else:
                # Atualizar apenas o texto
                try:
                    graph.put_object(parent_object=post_id, connection_name='', message=message)
                except Exception as update_error:
                    return {'error': f"Erro ao atualizar texto do post: {update_error}"}, 500

        # Fluxo para Instagram
        elif platform == "Instagram":
            return {'error': 'Edição de posts no Instagram não é suportada'}, 400

        # Plataforma Desconhecida
        else:
            return {'error': 'Plataforma desconhecida'}, 400
...
def edit_posts():
    try:
        ...
        post_id = request.form.get('id')  # ID do post no Facebook
        message = request.form.get('message')  # Mensagem atualizada
        media_id = request.form.get('media_url')  # ID da mídia no Facebook (fbid)

        # Verifica se o ID do post e a mensagem estão presentes
        if not post_id or not message:
            return jsonify({'success': False, 'error': 'Post ID e mensagem são obrigatórios.'}), 400

        # Verifica se o media_id é um ID válido antes de tentar atualizar o post
        if media_id:
            # Tentativa de buscar a mídia para validar o ID
            try:
                media_object = graph.get_object(id=media_id)
                if not media_object:
                    return jsonify({'success': False, 'error': 'ID de mídia inválido ou não encontrado.'}), 400
            except fb.GraphAPIError as e:
                return jsonify({'success': False, 'error': f'Erro ao validar mídia: {str(e)}'}), 400

        # Dados para atualização
        data = {'message': message}

        # Adiciona o attached_media apenas se media_id estiver presente e válido
        if media_id:
            attached_media = [{'media_fbid': media_id}]
            # Serializar attached_media como JSON string
            data['attached_media'] = json.dumps(attached_media)

        # Atualiza o post usando a API do Graph
        graph.put_object(parent_object=post_id, connection_name='', **data)

        return jsonify({'success': True, 'message': 'Post atualizado com sucesso.'})

    except fb.GraphAPIError as e:
        print(f"Erro ao atualizar o post: {e}")
...
def criar_campanha():
    try:
        ...
        orcamento_diario = request.form.get('orcamento_diario', '1000')  # Valor padrão
        data_inicio = request.form.get('data_inicio')
        data_fim = request.form.get('data_fim')
        post_id = request.form.get('post_id')  # ID do post no Facebook

        # Validar formato do post_id
        if not post_id or len(post_id.split('_')) != 2:
            flash("O ID do post não está no formato correto (PAGE_ID_POST_ID).", "danger")
            return redirect(url_for('listar_campanhas'))
        ...
        campanha_params = {
            ...
            'daily_budget': int(orcamento_diario) * 100,  # Valor em centavos
        }
        ...
        ad_set_params = {
            ...
            'daily_budget': int(orcamento_diario) * 100,
            'billing_event': 'IMPRESSIONS',
        ...
        if not ad_set or 'id' not in ad_set:
            flash("Erro ao criar conjunto de anúncios: retorno inválido da API", "danger")
            return redirect(url_for('listar_campanhas'))
        ...
        # Captura a resposta detalhada do erro, se disponível

        error_message = getattr(e, 'message', str(e))

        app.logger.error(f"Erro ao criar campanha: {error_message}")

        # Analisar resposta de erro se for JSON
        ...
        if hasattr(e, 'response') and e.response.content:
            ...
            try:
                ...
                detailed_message = error_data.get('error', {}).get('message', 'Erro desconhecido.')

                error_code = error_data.get('error', {}).get('code', 'Sem código.')

                error_subcode = error_data.get('error', {}).get('error_subcode', 'Sem subcódigo.')

                flash(f"Erro {error_code} (subcódigo {error_subcode}): {detailed_message}", "danger")
...
@app.route('/promote_posts', methods=['POST'])
def promote_posts():
    data = request.json
    ...
    try:
        ...
        ad_params = {
            ...
            "creative": {
                "creative_id": create_ad_creative(graph, post_id)
            },
            "adset_id": create_ad_set(graph),
        }

        ad_response = graph.put_object(
            parent_object=f"{AD_ACCOUNT_ID}/ads",
            connection_name="",
            **ad_params
        )

        return jsonify({"success": True, "data": ad_response})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


def create_ad_set(graph):
    ad_set_params = {
        "name": "Ad Set para Promoção",
        "campaign_id": create_campaign(graph),
        "daily_budget": "5000",  # Em centavos (R$50,00)
        "billing_event": "IMPRESSIONS",
        "optimization_goal": "POST_ENGAGEMENT",
        "targeting": {
            "geo_locations": {"countries": ["MZ"]},
    ...
    ad_set = graph.put_object(
        parent_object=f"{AD_ACCOUNT_ID}/adsets", connection_name="", **ad_set_params
    )
    return ad_set['id']


def create_campaign(graph):
    campaign_params = {
        "name": "Campanha para Promoção de Postagem",
        "status": "ACTIVE",
        "objective": "POST_ENGAGEMENT",
        "buying_type": "AUCTION",
    }
    ...
    campaign = graph.put_object(
        parent_object=f"{AD_ACCOUNT_ID}/campaigns", connection_name="", **campaign_params
    )
    return campaign['id']


def create_ad_creative(graph, post_id):
    creative_params = {
        "name": "Criativo para Promoção",
        "object_story_id": f"{PAGE_ID}_{post_id}",
    }

    creative = graph.put_object(
        parent_object=f"{AD_ACCOUNT_ID}/adcreatives", connection_name="", **creative_params
    )
    return creative['id']

# Função para obter estatísticas da página do Facebook
def get_facebook_page_insights():
    ...
    insights = graph.get_object(f"{PAGE_ID}/insights", **params)
    return insights

# Função para obter estatísticas da conta do Instagram
def get_instagram_account_insights():
    url = f"https://graph.facebook.com/v17.0/{INSTAGRAM_ACCOUNT_ID}/insights"
    params = {
        'metric': 'follower_count,website_clicks',
        'period': 'day',
        'access_token': ACCESS_TOKEN
    }
...
# Rota para obter estatísticas combinadas
@app.route('/insights', methods=['GET'])
def get_insights():
    facebook_data = get_facebook_page_insights()
    instagram_data = get_instagram_account_insights()

    return jsonify({
        'facebook_page_insights': facebook_data,
